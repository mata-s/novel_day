...
Deno.serve(async (req) => {
  ...
  try {
    const { memo, style, persona, freeContext, occupation } = await req.json();
...
重要な制約（必ず守ってください）:
- ユーザーのメモに書かれていない「具体的な事実」を勝手に作らないでください。
- 具体的な事実とは、場所（ベンチ、公園、駅、電車内など）、行動（座った／走った／誰かと話したなど）、会話の内容、登場人物の追加、食べ方・場所の詳細などです。
- 例えば、メモに「ミニストップに行った」「ご飯を食べた」とだけある場合、
  「ベンチに座って食べた」「公園の片隅で食べた」などと書くのは禁止です。
- 補ってよいのは、
  - 主人公の感情（うれしかった／ほっとした／少しさみしかった など）
  - 体の感覚（冷えた手／温かいご飯／少し疲れた足 など）
  - あいまいな情景（店内の光、空気の静けさ、にぎやかさ など）
  といった、「メモと矛盾せず、かつ新しい事実をでっちあげない」情報だけです。
...
- ユーザーのメモに時間帯（朝・昼・夜・夕方など）が書かれていない場合は、特定の時間帯を示す表現は避けてください（曖昧な時間の表現にとどめる）。

参考情報:
${typeof occupation === "string" && occupation.trim().length > 0 ? `- 仕事・役割: ${occupation}` : "- 仕事・役割: （未設定）"}
${typeof freeContext === "string" && freeContext.trim().length > 0 ? `- 日常の背景メモ: ${freeContext}` : "- 日常の背景メモ: （未設定）"}

これらの情報は、文体や心情、その日の空気感を考えるためのヒントとして使ってください。
ただし、次のルールに従って必ず検討し、可能なかぎり本文に反映してください。

- ユーザーのメモの内容と自然につながる場合は、
  「仕事・役割」や「日常の背景メモ」に含まれる情報を、本文のどこかで1回以上さりげなく使ってください。
- とくに、メモの中に「仕事」「バイト」「勤務」「シフト」「授業」など、
  その日の役割や行動に関する記述がある場合は、
  occupation を手がかりにした一文を必ず1回以上含めてください。
- 日常の背景メモ（freeContext）が設定されている場合は、
  そこに書かれている雰囲気やキーワードを参考にして、
  心情や空気感の描写に1回以上反映させてください。

ただし例外として、以下は許可します。

- ユーザーが入力した「仕事・役割（occupation）」に直接結びつく、
  一般的で誰でも想像できる結果や変化は、1〜2点まで描写してよい。

例:
- ホテル清掃 → 部屋が整う、ベッドがきれいになる
- コンビニ → 商品が並ぶ、レジを終える
- 事務仕事 → 画面を閉じる、書類をまとめる

これらは「新しい具体的事実の創作」には当たりません。
ただし、固有名詞・特定の人物・詳細な出来事は追加してはいけません。
